...
class StatsdReporter {
    ...
    start(err, args) {
        if (!this.options.destination) {
            console.log(`##statsd[testSuiteStarted error='Destination address is missing!']`);
        }
        if (!this.options.port) {
            console.log(`##statsd[testSuiteStarted error='Port is missing!']`);
        }
    ...
    item(err, args) {
        const metricname = this.metricname(this.currItem.name);
